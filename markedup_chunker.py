...
def markedup_chunker_decorator(func):
    def root_chunker(self_, chunk, add_buffer=True):
...
def add_to_buffer(text):
    ...
    if buffer_len + len_new_text > max_buff_size:
        ...
        if sent_pos > -1:
            sent_pos -= diff
    buffer_ += text
    log.debug(f"sent_pos: {sent_pos}, buffer: '{buffer_}'")
